"""citation_guard.py — deterministic, latency-free citation faithfulness for answers.

After synthesis, verify that every *value-bearing* claim (one that asserts an ID, number,
date, amount, or proper noun) cites a SOURCE whose text actually contains that value — using
the SAME token-boundary matcher the eval trusts (`eval_grading.answer_matches`). This catches
the dangerous failure mode (a confident value cited to a chunk that does not support it, e.g.
a family member's ID asserted as the owner's) with **zero extra LLM calls**.

Scope, stated honestly: this checks *verbatim* facts only — the things a wrong citation most
damages. Paraphrased/synthesised prose is NOT verified here; that is measured by the eval's
LLM judge (`run_eval.py --synthesis`). The guard *flags* (does not delete) unverified claims,
so a false positive costs a spurious "⚠ unverified" note, never a mangled answer — and a real
miss stays visible for the dogfood loop.
"""
...
from eval_grading import answer_matches  # token-boundary, FTS-aligned matcher
...
def audit(answer: str, cards) -> CitationAudit:
