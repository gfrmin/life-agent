...
## credence-pi (`../credence/apps/credence-pi`) — the brain (TS body + Julia daemon)
...
- `credence-proxy` (`../credence/apps/python/credence_router`, OpenAI-compatible router) could back
  pi-ai so the brain also routes *models*. `credence_agents` (`../credence/apps/python/credence_agents`, the Bayesian
  agent library) could inform policy. Both are separate today; integrate in Phase 2+ if useful.

## "credence-pi" naming
The owner's term: the **pi harness/agent** at `../credence/apps/credence-pi` used with
`../pi-mono`. Not a Raspberry Pi. The always-on runtime is just this app run as a `systemd --user`
