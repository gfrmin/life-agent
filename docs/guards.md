...
> demonstrated by mutation, never asserted by shape.**

Rows 12, 22 and 23 stood DEFEATED behind this question, and `r27` declined to patch them
because a fourth consecutive pass over one class fixes spellings, not the class. The rule is
deliberately **semantic**, because every one of row 22's three defeats worked the same way:
police the universe *syntactically* and any narrowing is escapable by re-spelling it. A
universe that must be **named and non-empty** cannot be silently widened or narrowed —
both change a number the guard is obliged to report.

The two clauses are what this repo's own working guards already do when they are any good:
`test_rulings_register.py` carries a positive control asserting its census finds ≥20
documents (the universe clause), and every predicate in `join_census.py` was verified RED by
mutation before it was allowed to read (the control clause). The ruling makes that the
standard rather than the habit. **G4, the adversary pass, is unblocked by it.**
...
| 12 | The instrument's own integrity (`test_collapse_record.py`) | the replay recorder does not lie about what it recorded | **instrumented** | — not attacked. **Under `G-3` its remaining debt is named: it carries no mutation witness.** Reaching *resolved* requires removing what it controls for and watching it turn RED |
...
pass over one class. **The method is now RULED (2026-08-31, `G-3` — see "The method"
above), so the rows are superseded rather than patched and G4 is unblocked**; they stay
DEFEATED until a guard implements the ruled clauses and a planted violation is watched
killing it. **Read the number with known-and-uncovered 17: r27's rows were earned
