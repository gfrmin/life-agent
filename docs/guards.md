...
| 20 | `_SKIP_PATHS` is pinned whole and every skip announced | a skip exempts a whole tracked file from every rule | **instrumented** | K2-16: adding `README.md` to the set exempted the front page with the gate green **DEFEATED at K3's G4:** the equality pin has the wrong universe — a SECOND skip set (`_BINARY_SUFFIXES`) is pinned by nothing and announced never, restoring K2-16's exact defect through a door the pin does not cover |
...
**12 rows resolved, 17 instrumented.** The register opened at four and twelve, and three of
...
15. **35 of K3's G4 claims were never verified, and the rows they touch are
