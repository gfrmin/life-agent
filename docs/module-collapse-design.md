...
| **M2** | **the one poster** (Q-O6): one driver function; `AC.answer`/`answer_via_executor` become thin delegating shims; the family leaves return their decision; the driver records once; A-3/D-10 die; S-1's unavailability path unified (B-2/A-1) | shims for **one checkpoint** (M2→M3), then deleted; the leaves' own `DEC.append`/`D.record` calls become dead code at M2 and are removed at M3 | 7.1; 7.2 (body equality incl. the new fields); **7.4** (decisions + §18.9 nodes: one write per decision) | the reach surface's decisions become priced in the ledger — a *known* record change, pre-registered: absent keys → present with `0.0`/`""` · **DONE 2026-08-25** (`docs/unification/reports/r12-collapse-m2.md`): one poster (`ask_client.post_decision`), one recorder (`core/recorder.py`), the §6.5 unavailability record (local append, `decision_id=""`, unfoldable — the bridge refuses an empty id); G1 2705 green + lint/type clean, G2 314/314 twice (105 direction-asserted; DIR-1 amended blind to cover the 102 posting A-loop bodies), G3 golden green both legs with counts identical, P5 live fingerprints byte-identical to r11; shims (`answer_via_executor`/`_edge_curves`) die at M3 |
| **M3** | **the one fold entry point** (Q-O5): `posterior(policy=…)`; D-8's five sites become callers; `fold_version` covers the policy; the memo keyed by it | none (a rename with a required argument; no old spelling survives) | 7.1; 7.2 (`policy` field); **7.3** in the frozen regime — Δ on the band | also the D-2 unification of the two wire reliability instances into `reliability(edge, cell)` — same checkpoint, same instruments; the calibration curve untouched (debt) · **DONE 2026-08-25** (`docs/unification/reports/r13-collapse-m3.md`): `posterior(brain, model, evidence, *, policy)` with the conditioning set structurally enforced (frozen-elicitations refuses projected events), `fold_version` covering the policy, six callers named; `core/reliability.py` holds the one prior table with both instruments as drift-gated bindings; `AC.answer`/`ask._edge_curves` deleted (the executor-lane surface is retained, not a shim — r13 amendment 4; the leaf write tails wait for M5's single caller — amendment 5). G1 2718 + clean lint/type; G2 314/314 pure equality; G3 = run 15 (`gate-20260825T215428`) **PASS 0.907/+0.421, zero NEW wrongs, $0.68** — run 14's numbers reproduced on the collapsed fold |
| **M4** | **the utility atom + the price table** (Q-O3, §4): `action_utilities`/`realised_utility` derived from `u_assert`; the table one module; `lambda_usd` from Ū only (E-5); the tier/menu constants relocated; E-3/E-8/E-9/BR-4's constants die into priors | none | 7.1; 7.2 (vectors identical on every fixture — the derivation is exact); 7.3 | the gate reads the same atom (offline) — its frozen δ/level untouched (§6.1) · **DONE 2026-08-26** (`docs/unification/reports/r14-collapse-m4.md`): `core/pricing.py` is THE table (PRICING_VERSION 2 — tiers, transform menu, deliberate seed, grow actuators, re-read model, and the D-2 reliability prior column relocated from its interim home); executor/gather/reliability BIND the same objects (drift-gated); `realised_utility`'s report branch spelled through `u_assert` (D-1, exact); E-5's two silent `lambda_usd` defaults die — the latent is REQUIRED and fails loud. G1 2724 + clean lint/type; G2 314/314 pure equality; G3 = run 16 (`gate-20260826T003710`) **PASS 0.961/+0.519, zero NEW wrongs, $0.60** — the headline's move off run 15 is one named row (the warm-deliberate wobble landing withheld). **Q8 decided: the M3 lane deletes at M5**; **the split signed: single M5** |
| **M5** | **the argmax absorption**: the driver's remaining choices die or absorb (E-1/L-9/B-6 family choice → rows; E-4 miss → reason; E-12 latch; L-3 scoped rows; GA-1/GA-2/`gather_answer` dies; M-1…M-5 the live lane dies; B-4/B-1/B-5 dispatch dies; the terminals-only regime declared) | `gather_answer` and the M3 lane deleted; the in-process families stay as **leaves** (not shims — they are the terminals-only regime's ranking) | 7.1; 7.2 on **every** fixture incl. terminals-only ones; **7.3**; 7.4 not needed (no store) | *argued adjustment (approved as written):* absorption is the largest move and depends on M2–M4; it splits into M5a (family choice + regimes) and M5b (grow-target absorption + live-lane retirement) if 7.2 shows the pair is not bisectable in one step — the reviewer signs the split at M4's report. **M4's report also decides Q8:** default *delete* the M3 lane; if the shadow's v2 differential needs `CO.map_action` to compute its counterfactual, it survives *renamed into the shadow namespace* as a measurement function — never as anything that could be read as a lane · **DONE 2026-08-26** (`docs/unification/reports/r15-collapse-m5.md`): single M5 as signed; the dispatch, the gather fork and the M3 live lane deleted; `core/terminals.py` the absorbed body; the terminals-only regime DECLARED (§6.5 kept for no-engine-at-all); L-3's scoped rows engine-picked; D-5's one withhold-reason derivation. G1 2704 + clean lint/type; G2 212/314 identical, the misses the sole DIR-L3-wire cassette class (restored by the post-M5 re-record); G3 = runs 17+18: run 17 (`gate-20260826T025059`) **FAIL 0.743/+0.238** priced A2's every-terminal grow offer as harmful (the engine's hand-set grow priors — foundations §14's hand-priced-VOI arc; owner ruling option A, conferral in-tree), A5 reverted the ENACTMENT alone (the probe's 62/63 stands as engine evidence), and run 18 (`gate-20260826T083356`) read **PASS on all four conjuncts — 0.959/+0.514, wrongs exactly the two standing rows, the wobble row withheld, $0.37** — reproducing run 16 almost exactly, the collapse attributed to the grow offer alone |
| **M6** | **the observation model declared once** (§3.3): the 36 clauses each with one home; D-11/D-14/D-15 one function each; E-7's replace-branch → second channel (gated on §9 Q3's measurement) | none | 7.1; 7.2 (identical decisions — declaring is not changing, except E-7 which is gated) | may interleave with M2–M5 per clause where a clause's two spellings block a move; E-7 waits for its measurement (Q3, endorsed: pre-registered off-gate, criteria frozen first); Q5's volatility transcript is produced here · **DONE 2026-08-26** (`docs/unification/reports/r16-collapse-m6.md`): every §3.3 clause one declaration/one home — D-11 (`BR._lattice_join` + the one observation builder, both edges bind), D-14 (`LK.source_date_iso`, the ≻-chain declared with its P-1/N-4 branches), D-15 (`RX.VERDICT_Y` naming all three branches + owner ≻ Claude precedence; the membrane binds is-identity), 23 uniform `[§3.3 · X-n]` stamps drift-gated, riders D-12 (`DEC.edge_id`) + D-13 (stack URLs read once); E-7 VERIFIED second-channel at every site (zero replace sites, each pinned by a named test); Q5's transcript produced ($0, 103/104 warm) and its disposition decided (§9 Q5). G1 2721 + clean lint/type; G2 **314/314 pure equality on m5-base**; no priced run demanded |
| **M7** | the register (§6) pinned by its tests; the vocabulary derivations (D-6) + label-views (D-4/D-5); config surface (D-13); **Appendix A signed here, not before** (the constitution changes when the structure exists) | — | 7.1 | the census's re-listing guard · **DONE 2026-08-26** (`docs/unification/reports/r17-collapse-m7.md`): the register pinned (census {6.1…6.13} → artefact pins, both predicates mutation-verified RED); D-6/D-4 closed as derived views; G1 2726 green + clean lint/type, advisory replay 314/314 pure equality on m5-base; **Appendix A SIGNED 2026-08-26** (owner keypress — `docs/unification/conferrals/appendix-a-conferral.md` carries the RULING): A.1–A.3 enacted verbatim onto PRINCIPLES §16/§15/§14. **The ladder is CLOSED** |
...
- **Q5 (R) — volatility: override or combine, table or latent?** BR-1 lets the half-life
  ...
  transcript decides.** **RESOLVED by the transcript (r16, 2026-08-26, $0):** 89 of 103
  routed questions disagree (86.4%), ALL one-directional (model-permanent →
  table-decays; the risk direction empty), and 65 of the 89 sit on the DEFAULT branch
  — permanent-in-kind constructs decayed by the unclassified fallback, so the
  disagreements are NOT all the table's wins and **a latent with a prior IS
  warranted**: registered as future work (a learned per-construct volatility
  latent under its own pre-registration); the override stays deployed until it
  lands. Full table (corpus-derived construct strings) out of tree:
  `$LIFE_AGENT_KB/eval/q5-volatility-transcript-20260826.md`.
