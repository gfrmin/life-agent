...
| interval claim | on a `quantity` question the answer may be a RANGE (`lookup.GRAMMAR["report_interval"]`, r30b): `Between <lo> and <hi> — credence <p>`. The endpoints are the candidates' OWN display strings — never a reformatted number, so no precision or currency the corpus did not carry is invented — and the credence is the posterior mass the range covers, so a wider claim visibly buys its confidence. It is the same speech act as a report (an assertion of the quantity) at a lower precision, chosen by the argmax against the crisp reports and the withholdings, never composed after one |
