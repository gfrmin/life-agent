...
| **M-27 · restarting a service is a measurement, not a formality** | 2026-09-02 | `GD-20` | A restart taken to make a merged change live **is an experiment on the deployed box**, and it must be verified afterwards on the thing it was supposed to start — not on `ActiveState=active`, which stayed green throughout while the shadow died. r46's restart permanently killed the form (cold Julia precompile 112 s + boot vs a 120 s ready-sentinel timeout; `max_respawns=3` exhausted in 9 minutes; ready arrived **3 s** after the last attempt gave up), reproducing `r45` C8's silent-death mode within an hour of publishing a report about it. **Say what you restarted, watch the artefact it feeds until it appears, and publish the cost.** Cousin of `M-19` — that says a launcher restores the tree it found; this says the operator confirms the thing came back. |
| **M-28 · a measurement pins its tree for the whole run, not just at launch** | 2026-09-02 | `r46b-grid-precision` deviation 5 | Editing the tree while a measurement against it is running **silently turns the baseline into the treatment**. r46 leg B launched a decision-equality run and landed the `theta_grid` snap 50 seconds later; Python imports at process start, so the run compared the snapped grid against itself and would have reported *zero differing decisions* — the very result the leg wanted — for the wrong reason. Caught by comparing the python process's start time against the file mtime, not by anything in the output, which looked perfect. **A long measurement holds the tree read-only until it finishes, and a run that cannot prove which tree it loaded is void.** Cousin of `M-19` (a launcher restores the tree it found) and `M-11` (pin the tree) — those bracket the run; this one is about its middle. |
...
| **M-29 · never mutate over uncommitted work** | 2026-09-03 | `r46c-act-conditioning` §Disclosure 1 | A mutation battery that restores each mutant with `git checkout -- <file>` **reverts uncommitted changes to that file** — run against an uncommitted rewrite, its final restore silently discards the rewrite, and the next commit captures only whatever else was staged. r46c's K7 battery wiped the instrument rewrite this way; the prior commit landed the test change alone, and the mismatch surfaced only on re-reading the committed tree. **Commit the code under test before running a git-checkout mutation harness over it.** Cousin of `M-10` (rehearse in a worktree, retain the transcript) and `M-28` (a measurement pins its tree for the whole run): those protect a measurement's tree; this protects the tree a verification harness edits and restores. |
| **M-30 · inert on the observed range is not inert on the boundary** | 2026-09-04 | `r48-reearn-measurement` §J3 (`GD-25`) | A change verified as decision-inert across every *recorded* input can still move a **structural boundary** lying outside that range. `r46` leg B snapped the θ grid to a 2^-20 lattice for speed and verified it honestly — 428 summaries, **zero** differing actions, `p1` gap ~3e-7 — every claim true, and none of it touched the top rung, because no episode approaches it. But `theta_grid` seeds a rung at each `argmax_crossings` value and `respond_threshold` returns the last of them, so the top rung **is** the decisive crossing: the snap rounded it **up** past the bar (+1.2e-8), voiding §16 finding 3's *unreachable-by-construction* argument two checkpoints before anyone read it. **When a change edits a rule that also defines a limit, verify the limit as well as the observed rows** — a structural claim is decided at a boundary the corpus never visits, so "zero rows changed" is not evidence about it. Cousin of `M-24` (a conditional in the register is a trigger you own) and `M-7` (read the deployed rule, never re-implement the constant it prices). |
