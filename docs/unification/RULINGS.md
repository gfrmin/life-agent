...
| **M-20 · measure the claims you are standing on** | 2026-08-31 | `r40-arc-c-preconditions` | A checkpoint's **first act** is to verify the inherited claims its plan rests on — installed binaries, live flags, streams a doc says are accruing. r40 was about to plan an arc on membrane-shadow §18's *"the shadow keeps accruing"*, which stopped being true three weeks earlier; the check cost four commands. Third instance in three checkpoints of building on an unverified premise (r36 from an instrument property, r39 from a disclaimed function, r40 from a doc sentence). |
