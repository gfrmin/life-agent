...
QS = {"q2-105": {"id": "q2-105", "answer": "(852) 5550 0187",
                 "answer_variants": ["5550 0187"]}}


def test_plan_supersedes_only_rows_whose_grade_moved() -> None:
    events = [
        _ev("q2-105", "(852) 5550 0143", "CORRECT", tx="t0"),          # stale: the tel
        _ev("q2-105", "(852) 5550 0187", "CORRECT", lineage=("L2",)),  # already right
        _ev("q2-105", "nothing", "INCORRECT", lineage=("L3",)),        # already right
        _ev("q2-001", "(852) 5550 0143", "CORRECT", lineage=("L4",)),  # other question
    ]
...
def test_plan_regrades_the_row_in_force_not_an_already_superseded_one() -> None:
    ...
    events = [_ev("q2-105", "(852) 5550 0143", "CORRECT", tx="t0"),
              _ev("q2-105", "(852) 5550 0143", "INCORRECT", tx="t1")]
...
def test_plan_names_lineage_less_rows_it_cannot_supersede() -> None:
    events = [_ev("q2-105", "(852) 5550 0143", "CORRECT", lineage=())]
    to_append, unfixable = RG.plan_regrades(events, QS, reason="x", run_id="r2")
    assert to_append == [] and [e.claim for e in unfixable] == ["(852) 5550 0143"]


def test_plan_ignores_non_edge_graders() -> None:
    events = [_ev("q2-105", "(852) 5550 0143", "CORRECT", grader="eval_lookup")]
...
def test_appended_regrade_supersedes_in_the_curve_fold(tmp_path: Path) -> None:
    ...
    O.append(log, _ev("q2-105", "(852) 5550 0143", "CORRECT", p=0.97))
...
def test_cli_dry_run_writes_nothing(tmp_path: Path, capsys) -> None:
    ...
    log = tmp_path / "outcomes.jsonl"
    O.append(log, _ev("q2-105", "(852) 5550 0143", "CORRECT"))
