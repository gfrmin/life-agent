...
# --- r10 D2: the entity key, applied at the base mint --------------------------------------

_KEY_Q = "what is the coverage for the WidgetFactory class?"   # PII-OK: synthetic identifier


def test_entity_key_drops_the_observation_whose_chunk_never_names_the_entity(
        migrated_root: Path) -> None:
    """The block this targets: the gold's carrier is a terse row that STATES the entity, the
    competitor's is a discursive passage about the same file that never names it. An exact
    typed key separates them where vocabulary overlap could not."""
    client = FakeClient([{"found": True, "value": "100", "quote": "WidgetFactory 100"},
                         {"found": True, "value": "0", "quote": "the module 0"}])
    hits = [_hit("a" * 64, "WidgetFactory 100"),        # PII-OK: synthetic
            _hit("b" * 64, "the module 0")]             # PII-OK: synthetic
    obs, _ind = observe_hits(migrated_root, _KEY_Q, hits, client=client)
    assert [o.value_norm for o in obs] == ["100"]


def test_entity_key_is_a_no_op_when_no_chunk_names_the_entity(migrated_root: Path) -> None:
    """`if any observation key_hits` — the filter must never empty a channel. A question whose
    entity appears in no chunk keeps every observation it had."""
    client = FakeClient([{"found": True, "value": "100", "quote": "coverage 100"},
                         {"found": True, "value": "0", "quote": "the module 0"}])
    hits = [_hit("c" * 64, "coverage 100"), _hit("d" * 64, "the module 0")]
    obs, _ind = observe_hits(migrated_root, _KEY_Q, hits, client=client)
    assert sorted(o.value_norm for o in obs) == ["0", "100"]


def test_entity_key_is_a_no_op_when_the_question_names_nothing(migrated_root: Path) -> None:
    """60% of the corpus. The key cannot fire, and the channel must be untouched."""
    client = FakeClient([{"found": True, "value": "5", "quote": "5 days"},
                         {"found": True, "value": "7", "quote": "7 days"}])
    hits = [_hit("e" * 64, "5 days"), _hit("f" * 64, "7 days")]
    obs, _ind = observe_hits(migrated_root, "how many days?", hits, client=client)
    assert sorted(o.value_norm for o in obs) == ["5", "7"]
