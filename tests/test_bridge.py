...
# --- r09b T2: the nested-dependence collapse -------------------------------------------------


def _synth(reports: int, authority: float) -> dict:
    """A synthesised probe confirm: no verbatim quote, no document of its own."""
    return {"reports": reports, "group": 0, "authority": authority, "subject_factor": 1.0,
            "time_factor": 1.0, "competition_factor": 1.0,
            "quote": "", "doc_key": "", "value_norm": "alpha"}


def test_synthesised_confirms_of_one_candidate_collapse_to_one_witness() -> None:
    """r09b T2: the tiers and the deliberate edge re-read the SAME documents — a re-read
    cannot corroborate itself. Stacked synthesised confirms of one candidate collapse to
    the max-covariate witness (run 13's wrong rows all carried this stacking signature)."""
    from life_agent.bridge.observations import join_wire_observations

    channel = [_wire("Alpha", "d0", quote="ctx alpha attested")]
    joined = join_wire_observations(channel, [_synth(0, 0.8)], ["Alpha"])
    joined = join_wire_observations(joined, [_synth(0, 1.0)], ["Alpha"])
    joined = join_wire_observations(joined, [_synth(0, 0.9)], ["Alpha"])
    assert len(joined) == 2  # the base witness + ONE synthesised witness
    synths = [o for o in joined if not o["doc_key"]]
    assert len(synths) == 1 and synths[0]["authority"] == 1.0  # the max-covariate survivor


def test_synthesised_disagreeing_candidates_all_survive() -> None:
    """T2 collapses re-reads of the SAME candidate only — disagreement between synthesised
    reads is signal, never collapsed."""
    from life_agent.bridge.observations import join_wire_observations

    channel = [_wire("Alpha", "d0", quote="ctx alpha attested")]
    a = _synth(0, 0.9)
    b = {**_synth(1, 0.9), "value_norm": "bravo"}
    joined = join_wire_observations(channel, [a], ["Alpha", "Bravo"])
    joined = join_wire_observations(joined, [b], ["Alpha", "Bravo"])
    assert len(joined) == 3


def test_document_carrying_observations_never_collapse_under_t2() -> None:
    """T2 touches only observations with no document identity: two genuine attestations of
    one value in two documents (value-only quotes — §5 keeps them too) both survive."""
    from life_agent.bridge.observations import join_wire_observations

    channel = [_wire("Alpha", "d0", quote="Alpha"), _wire("Alpha", "d1", quote="Alpha")]
    joined = join_wire_observations(channel, [_synth(0, 1.0)], ["Alpha"])
    assert len(joined) == 3
